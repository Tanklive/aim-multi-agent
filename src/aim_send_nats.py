...
def _get_credentials(from_id: str) -> str:
    """从 aim.json 读取指定 agent 的 JWT creds 路径，自动回退 token"""
    ...
    agent_cfg = agents.get(from_id, {})
    creds_path = agent_cfg.get("creds_path", "")
    if creds_path:
        creds_path = os.path.expanduser(creds_path)
        if os.path.isfile(creds_path):
            return creds_path
    # 回退：全局 nats_jwt_path → nats_token
...
async def send_private(from_id: str, to_id: str, content: str, nats_url: str):
    """使用 SDK 发送私聊消息"""
    creds = _get_credentials(from_id)
...
async def send_request(from_id: str, to_id: str, content: str, timeout: int, nats_url: str):
    ...
    creds = _get_credentials(from_id)
    nc_kwargs = {"servers": [nats_url]}
    if creds:
        if os.path.isfile(creds):
            nc_kwargs["user_credentials"] = creds
        else:
            nc_kwargs["token"] = creds
...
async def main():
    ...
    parser.add_argument("--from", dest="from_id", default="ZS0002", help="发送者 ID")
