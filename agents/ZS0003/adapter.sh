#!/bin/bash
set -eu
# v1.14.3: PROBE_TIMEOUT 120s (止血) + exit=124 首次内部重试一次 (治根)，不透传
...
_dispatch_with_new_conv() {
    ...
    while [ $_attempt -le $_max_attempts ]; do
        ...
        if [ $rc -eq 124 ] && [ $_attempt -lt $_max_attempts ]; then
            echo "[letta-adapter] exit=124 首次重试 (attempt $_attempt/${_max_attempts})" >&2
            sleep 2
            > "$_tmp_out"  # 清空临时文件准备重试
            _attempt=$((_attempt + 1))
            continue
