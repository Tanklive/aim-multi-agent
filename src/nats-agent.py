#!/usr/bin/env python3
"""AIM Agent NATS — 通用标准接入端 (Veritas 协议 v2.0 统一标准)

所有 AIM Agent 使用同一份 nats-agent.py，通过命令行参数区分身份。

启动（每个 Agent 传自己的 ID/名称/框架/emoji）:
    # ZS0001  呱呱 (openclaw)
    python3 nats-agent.py --agent-id ZS0001 --agent-name 呱呱 --framework openclaw --emoji 🐸
    # ZS0002  吉量 (hermes)
    python3 nats-agent.py --agent-id ZS0002 --agent-name 吉量 --framework hermes --emoji 🐴
    # ZS0003  小火鸡儿 (letta)
    python3 nats-agent.py --agent-id ZS0003 --agent-name 小火鸡儿 --framework letta --emoji 🐤 --letta-agent-id agent-local-xxx
...
架构（纯中继模式）:
    ...
    ├── 消息队列 + Trigger (文件管道 → 各框架主会话 AI 处理)
    ├── 回复轮询 (.aim-replies/ → NATS 发送)
    └── Observer 事件 / 心跳

设计原则：
    - nats-agent 只做消息中继（收→队列→轮询reply→发送），不直接调用 AI
    - AI 处理由各 Agent 框架的主会话心跳完成
    - 统一配置 ~/.aim/config/aim.json，各 Agent 匹配自己的 ID
    - 通用标准，不绑定特定工具（Ollama 等本地模型不可行）

Protocol: AIM Veritas v2.0
"""
...
class AIMAgentNATS:
    ...
    def __init__(
        ...
        self.log.info(f"🚀 {emoji} AIM Agent NATS v2.0 初始化 ({self.framework})")
        ...
        # Letta 不走 FrameworkCLI，直接用 subprocess
        if self.framework == "letta":
            self._fw_cli = None
            if self.letta_agent_id:
                self._verify_letta_agent()
        else:
    ...
    def _verify_letta_agent(self):
        """启动时验证 Letta agent 是否存在（健康检查第1层）"""
        ...
        try:
            ...
            if self.letta_agent_id in result.stdout:
                self.log.info(f"✅ Letta agent 验证通过: {self.letta_agent_id}")
            else:
                self.log.warning(f"⚠️ Letta agent ID ({self.letta_agent_id}) 未在列表中")
        except Exception as e:
            self.log.warning(f"⚠️ Letta agent 验证失败: {e}")
    ...
    async def _process_message(
        ...
        async with self.semaphore:
            try:
                ...
                # 写 trigger 文件通知主会话心跳
                trigger_file = os.path.join(str(Path.home()), '.openclaw', 'workspace', '.aim-trigger')
                try:
                    Path(trigger_file).write_text(json.dumps({"last_msg": msg_id, "ts": time.time()}))
                except Exception:
                    pass
                log.info(f"📝 [{msg_id[:8]}] 入队 + trigger，等候主会话处理")

                # 不自动 ACK（避免消息循环）；由主会话 AI 处理回复
...
def main():
    ...
    parser.add_argument("--nats-url", "--server", default="nats://127.0.0.1:4222", help="NATS Server URL")
    parser.add_argument("--emoji", default="🐴", help="Agent emoji")
    parser.add_argument("--letta-agent-id", "--letta-agent", default="", help="Letta agent ID (仅 letta 框架需要)")
