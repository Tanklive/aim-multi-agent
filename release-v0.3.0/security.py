...
class SecurityManager:
    ...
    def load_secret(self, agent_id: str) -> str | None:
        """加载密钥（优先缓存，其次文件）"""
    ...
    def verify_timestamp(self, timestamp: int, window: int | None = None) -> bool:
        """验证时间戳在允许窗口内"""
        if window is None:
    ...
    def generate_message_signature(self, agent_id: str, msg_id: str, content: str, timestamp: int | None = None) -> tuple[int, str]:
