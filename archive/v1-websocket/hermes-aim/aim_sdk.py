...
任何框架（LangChain/AutoGen/Claude Code/Eliza/...）只需要实现：
  ...
  2. 能接收 WebSocket 消息（或在 AIM Server 上注册 callback）
...
class AIMAgent:
    ...
    def __init__(self, agent_id: str, name: str = "", server_url: str = "ws://127.0.0.1:18900",
    ...
    async def _listen_loop(self):
        """通过 WebSocket 直连 AIM Server 监听消息"""
        while True:
            try:
                async with ws_connect(self.server_url, open_timeout=10, proxy=None) as ws:
                    # 认证
                    ...
                    print(f"✅ {self.emoji}{self.name}({self.agent_id}) 已连接 AIM Server")
