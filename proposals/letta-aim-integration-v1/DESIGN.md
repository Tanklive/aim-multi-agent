...
## 2. 标准方案：aim-letta-adapter v1.1（3 组件）

### 本次更新 (v1.1)
- ✅ 路径可配置：支持 `--queue-dir` / `--reply-dir` / 环境变量
- ✅ 默认路径标准化：`~/.aim/agents/{id}/queue` (独立于 openclaw workspace)
- ✅ 兼容 nats-agent V2 当前写入路径
- ✅ idle 降频逻辑：检测到消息立即恢复 2s (吉量建议)
- ✅ 跨平台说明：macOS launchd / Linux systemd
- ✅ 旧脚本清理：deprecate `aim-queue-*` 系列
...
| 平台 | 守护进程管理 | watcher 兼容性 |
|------|-------------|---------------|
| macOS | launchd plist | ✅ 已验证 |
| Linux | systemd service | ✅ 兼容（watcher 是纯 Python） |
| Windows | NSSM / Task Scheduler | ⚠️ 未测试 |

systemd service 示例：
```ini
[Unit]
Description=AIM Letta Queue Watcher for {agent_id}
After=network.target

[Service]
Type=simple
ExecStart=/usr/bin/python3 -B /home/user/.aim/agents/{agent_id}/aim-letta-watcher.py --agent-id {agent_id}
Restart=always
