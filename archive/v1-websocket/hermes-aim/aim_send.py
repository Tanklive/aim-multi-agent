...
2. 轻量级：不启动 node.py，直接 WS 连 AIM Server → 认证 → 发消息 → 断开
...
# no_proxy 必须含 .local 域名，否则 mDNS 连接可能被代理拦截
...
def get_server_url(config: dict, use_tls: bool = None) -> str:
    """自动发现 AIM Server 地址，支持域名模式（局域网 mDNS 模拟公网）

    优先级:
      1. AIM_SERVER_URL 环境变量（最高优先级，手动指定）
      2. config.domain 字段（域名模式，走 wss://）
      3. admin 节点的 host:port（127.0.0.1 直连）
      4. 默认 ws://127.0.0.1:18900

    参数:
        use_tls: True 强制 wss://，False 强制 ws://，None 自动判断
    """
    # 1) 环境变量覆盖
    ...
    # 2) config.domain 字段（模拟公网域名）
    domain = config.get("domain", "")
    if domain:
        # 检查 Server 的 wss 端口是否启用
        wss_port = config.get('security', {}).get('wss_port', 18901)
        scheme = "wss" if use_tls is not False else "ws"
        return f"{scheme}://{domain}:{wss_port if scheme == 'wss' else 18900}"
...
async def send_message(from_agent: str, to_agent: str = None, content: str = "",
                       ...
                       channel: str = "script", use_tls: bool = None) -> dict:
    """
    发送一条消息到 AIM 系统
    ...
    参数:
        ...
        group_id: 群组 ID（群聊模式）
        timeout: 连接超时秒数
        use_tls: True 强制 wss，False 强制 ws，None 自动

    返回:
        {"success": bool, "msg_id": str, "detail": str}
    """
    config = load_config()
    server_url = get_server_url(config, use_tls=use_tls)
    ...
    try:
        ...
        async with ws_connect(server_url, open_timeout=timeout, ssl=ssl_ctx, proxy=None) as ws:
            # Step 1: 认证
        ...
        return {"success": False, "msg_id": msg_id,
                "detail": f"无法连接 AIM Server ({server_url}): Server 未运行或端口不通"}
    except asyncio.TimeoutError:
        return {"success": False, "msg_id": msg_id,
                "detail": f"连接 AIM Server 超时 ({server_url})"}
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        print(f"")
        print(f"环境变量:")
        print(f"  AIM_SERVER_URL=wss://domain:18900    覆盖 Server 地址")
        print(f"")
        print(f"示例:")
    ...
    if is_group:
        ...
        result = asyncio.run(send_message(from_id, to_id, content=content, timeout=timeout, channel=channel, use_tls=use_tls))
