...
# ── Message/Task 分层（P1 Schema v1）────────────────────


@dataclass
class AIMMessage:
    """AIM 消息（Chat）— 即发即收，不跟踪状态

    对应方案 unified-v1.md Message Layer Chat。
    适用于：普通对话、通知、广播。
    """
    msg_id: str = ""
    from_id: str = ""
    to_id: str = ""
    msg_type: str = "dm"  # dm | grp | request
    content: str = ""
    reply_to: str = ""
    ts: float = 0.0
...
class AIMTask:
    ...
    对应方案 unified-v1.md Message Layer Task。
    适用于：代码执行、文件分析、多步协作。
    """
    task_id: str = ""
    owner: str = ""
...
def load_global_config() -> dict:
    """加载 ~/.aim/config/aim.json 全局配置，环境变量优先覆盖

    环境变量覆盖表:
        AIM_NATS_URL → nats_server
        AIM_DEFAULT_GROUP → default_group
        AIM_TRUSTED_PEERS → trusted_peers（逗号分隔）

    Returns:
        配置 dict，文件不存在或解析失败时返回空 dict
    """
    cfg = {}
    ...
    if cfg_path.exists():
        try:
            ...
            log.warning(f"Failed to load aim.json: {e}")
            cfg = {}

    # 环境变量覆盖
    env_nats = os.environ.get("AIM_NATS_URL", "")
    if env_nats:
        cfg["nats_server"] = env_nats

    env_grp = os.environ.get("AIM_DEFAULT_GROUP", "")
    if env_grp:
        cfg["default_group"] = env_grp

    env_peers = os.environ.get("AIM_TRUSTED_PEERS", "")
    if env_peers:
        cfg["trusted_peers"] = [p.strip() for p in env_peers.split(",") if p.strip()]
...
class AIMNATSClient:
    ...
    def __init__(
        ...
        # Agent Card - standard citizen identity (P0 Schema)
    ...
    async def load_agent_card(self, card_path: str = "") -> Dict[str, Any]:
        """Load Agent Card from file or adapter.sh info

        Schema v1 (see unified-v1.md section 5.6):
          serial, name, client, runtime, execution_model,
          lifecycle, delivery, capabilities, network
        """
        card = {}
        ...
        if card_path:
            try:
                with open(card_path) as f:
                    card = json.load(f)
                log.info(f"Agent Card loaded from {card_path}")
            except Exception as e:
                log.warning(f"Agent Card file not loaded: {e}")

        # 2. Get runtime info from adapter.sh info
        try:
            info = await self._get_runtime_info()
            if info:
                card.setdefault("runtime", {}).update(info.get("runtime", {}))
                if "execution_model" in info:
                    card["execution_model"] = info["execution_model"]
                if "provider" in info:
                    card.setdefault("runtime", {})["provider"] = info["provider"]
                if "version" in info:
                    card.setdefault("runtime", {})["version"] = info["version"]
        except Exception as e:
            log.debug(f"runtime info not available: {e}")

        # 3. Ensure required fields
        card.setdefault("serial", self.agent_id)
        card.setdefault("name", self.agent_id)
        card.setdefault("client", {"type": "aim-client", "version": "1.0.0"})
        card.setdefault("execution_model", "realtime")
        card.setdefault("lifecycle", "AVAILABLE")
        card.setdefault("capabilities", [{"name": "chat"}])
        card.setdefault("delivery", {"mode": "realtime", "max_concurrency": 1})

        self.agent_card = card
        return card

    async def _get_runtime_info(self) -> Dict[str, Any]:
        """Get cached runtime info from adapter.sh info"""
        if self._runtime_info:
            return self._runtime_info
        return {}

    def set_runtime_info(self, info: Dict[str, Any]):
        """Set runtime info from external caller (e.g. nats-agent-v3)"""
        self._runtime_info = info
        if info:
            log.info(f"Runtime info: {info.get('provider', '?')} v{info.get('version', '?')} ({info.get('execution_model', '?')})")

    # -- NATS callbacks -------------------------------------------

    async def _on_nats_error(self, e):
        """NATS 连接错误回调"""
        if isinstance(e, str) and "bool object is not callable" in str(e):
            return
