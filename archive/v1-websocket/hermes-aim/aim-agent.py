...
# ===== 启动时加载环境变量（必须放在最前面，确保 subprocess 继承）=====

def load_env_file():
    """加载 ~/.hermes/.env 文件到环境变量"""
...
from status_feedback import StatusFeedback, init_status_feedback, get_status_feedback
...
class AIMAgentDaemon:
    ...
    def __init__(self, agent_id: str, token: str, server: str,
        ...
        import queue as msg_q; self._queue = msg_q.PriorityQueue()
        ...
        self._term: int = 1  # V2: term 机制，断连重连时递增
        self._is_handler: bool = False  # V2: 是否为 handler
        
        # Status Feedback 模块
        self._status_feedback = init_status_feedback(agent_id)
        ...
        while self._running:
            ...
            try:
                ...
                async with connect(
                    ...
                    # 设置 status_feedback 的 WS 发送函数
                    async def _ws_sender(feedback):
                        if self._ws and hasattr(self._ws, "state") and self._ws.state == 1:
                            await self._ws.send(safe_json_dumps(feedback))
                    self._status_feedback._ws_sender = _ws_sender
                    ...
                    async def _send_heartbeat():
                        while True:
                            ...
                            try:
                                ...
                                print(f"💓 心跳已发送", flush=True)
                            except Exception as e:
                                print(f"⚠️ 心跳发送失败: {e}", flush=True)
                                break
                    ...
                    async for raw in ws:
                        ...
                        try:
                            ...
                            if cmd == "promote_to_handler":
                                ...
                                self.log.info(f"🔄 被提升为 handler (原因: {reason}, 前任: {previous_handler})")
                                self._is_handler = True
                                continue

                            elif cmd == "demote_from_handler":
                                # V2: 处理 handler 降级通知
                                reason = data.get("reason", "unknown")
                                self.log.info(f"⬇️ 被降级为非 handler (原因: {reason})")
    ...
    async def _run_health(self):
        """health channel — 发送 AIM 协议心跳"""
        while self._running:
            await asyncio.sleep(15)  # 每15秒发送一次心跳
            # 只检查 main 连接是否活着
            if not self._ws:
                log.info(f"[health] main 连接已断，health 退出")
                break
            # 发送 AIM 协议层面的心跳
            try:
                heartbeat_msg = {
                ...
                log.debug(f"[health] 心跳已发送")
            except Exception as e:
                log.warning(f"[health] 心跳发送失败: {e}")
    ...
    async def _process_incoming(self, msg: dict):
        ...
        # 2.7.1 Phase 2.1: 发送 application 层 received ACK
        asyncio.create_task(self._send_ack(msg_id, "received", to_agent=sender))
        ...
        # 5. 入队
        self._queue.put(msg, priority)
        log_task_progress(msg_id, "入队", "success", f"优先级: {priority} | 队列: {self._queue.qsize()}")
    ...
    async def _process_with_semaphore(self, msg: dict, sender: str,
        ...
        async with self._semaphore:
            try:
                ...
                # Status Feedback: 任务开始
                sf = get_status_feedback()
                if sf:
                    await sf.start_task(msg_id)
                ...
                # Phase 2.1: 发送 application 层 read ACK（AI 开始处理）
                asyncio.create_task(self._send_ack(msg_id, "read", to_agent=sender))
                ...
                # Status Feedback: AI 调用开始
                if sf:
                    await sf.step_start(msg_id, "ai_call", "正在调用 AI 处理消息...")
                
                ai_response = await self._call_ai(content, context, sender, priority)
                
                # Status Feedback: AI 调用结束
                if sf:
                    await sf.step_end(msg_id, "ai_call", success=bool(ai_response), progress="AI 调用完成" if ai_response else "AI 调用失败")
                ...
                # Status Feedback: 任务结束
                if sf:
                    await sf.end_task(msg_id, success=True, summary=f"已回复 {sender}")
                ...
                # Status Feedback: 超时
                if sf:
                    await sf.end_task(msg_id, success=False, summary="处理超时")
                # 发送 timeout 状态
                asyncio.create_task(self._send_status_update(
                    msg_id, "timeout", "处理超时",
                    is_group=is_group,
                ...
                # Status Feedback: 错误
                if sf:
                    await sf.end_task(msg_id, success=False, summary=str(e)[:100])
                # 发送 error 状态
                asyncio.create_task(self._send_status_update(
    ...
    async def _call_cli(self, prompt: str, timeout: int = None) -> dict:
        ...
        # 长任务保活：每 15s 发 still_working 直到 AI 返回
        async def _keep_alive():
            sid = getattr(self, '_current_msg_id', '')
            if not sid:
                return
            waited = 0
            while waited < timeout:
                ...
                waited += 15
                await self._send_status_feedback(sid, "still_working", "running", f"AI 处理中，已等待 {waited}s", waited * 1000)
    ...
    async def _call_ai(self, message: str, context: str, sender: str, priority: str = "medium") -> str:
        ...
        # 状态回推：开始处理
        session_id = msg_id if hasattr(self, '_current_msg_id') else str(int(time.time()))
        await self._send_status_feedback(session_id, "task_start", "running", f"开始处理来自 {sender} 的消息", 0)
        ...
        if response.success:
            ...
            if response.session_id:
                self._current_session_id = response.session_id
            asyncio.ensure_future(self._send_status_feedback(
                session_id, "task_end", "completed",
                f"处理完成，回复内容: {response.text[:80]}...",
                response.duration_ms if hasattr(response, 'duration_ms') else 0
            ))
    ...
    async def _send_status_feedback(self, session_id: str, step: str, status: str, progress: str = "", duration_ms: int = 0):
        """发送 AI 处理状态回推到 AIM Server（observer 可见）"""
        if not self._ws:
            return
        try:
            ...
            log.warning(f"status_feedback 发送失败: {e}")
    ...
    async def _send_ack(self, msg_id: str, status: str, to_agent: str = "", detail: str = ""):
        """Phase 2.1: 发送 application 层 ACK 到 Hub
        ...
        try:
            ...
            if hasattr(self._ws, "state") and self._ws.state == 1:  # 1 = OPEN
                await self._ws.send(safe_json_dumps(ack_msg))
                log.debug(f"📨 ACK {status}: {msg_id[:12]}")
        except Exception as e:
            log.debug(f"⚠️ ACK 发送失败: {e}")
    ...
    async def push_to_bridge(self, msg: dict):
        ...
        try:
            ...
            # 格式化消息
            formatted_msg = f"[AIM] {sender}: {content}"
            
            # 调用 openclaw message send 推送到 Telegram
            cmd = [
            ...
            if result.returncode == 0:
                log.info(f"[bridge] 消息已推送到 Telegram: {sender}")
            else:
                log.warning(f"[bridge] 推送到 Telegram 失败: {result.stderr}")
                
        except Exception as e:
            log.warning(f"[bridge] 推送到 Telegram 失败: {e}")
...
def main():
    ...
    parser.add_argument("--server", default="ws://127.0.0.1:18900",
                        help="AIM server URL (default: ws://127.0.0.1:18900; env AIM_SERVER_URL overrides)")
    parser.add_argument("--token", help="Auth token (or from tokens.json)")
    args = parser.parse_args()

    # 优先级: 环境变量 > --server 参数 > config.domain
    if os.environ.get("AIM_SERVER_URL"):
        args.server = os.environ["AIM_SERVER_URL"]
    else:
        config_file = BASE_DIR / "config.json"
        if not config_file.exists():
            config_file = Path.home() / ".hermes" / "aim" / "config.json"
        if config_file.exists():
            try:
                with open(config_file) as f:
                    cfg = json.load(f)
                domain = cfg.get("domain", "")
                if domain and "://" not in args.server:
                    # 只有用户没显式指定 --server 时才用 domain
                    args.server = f"wss://{domain}:18900"
