...
def build_register_payload(agent_name: str, framework: str, operator_name: str = "",
                          operator_id: str = "OP0001") -> dict:
    """构建注册请求载荷
    
    Args:
        agent_name: Agent 名称
        framework: 框架名称 (custom/hermes/openclaw/letta等)
        operator_name: 操作人名称
        operator_id: 操作人 ID (默认 OP0001=大哥)
    """
    return {
        "cmd": "register",
        "operator": {"id": operator_id, "name": operator_name or agent_name, "contact": ""},
...
async def register_agent(server: str, agent_name: str, framework: str,
                          operator_name: str = "", operator_id: str = "OP0001") -> dict:
    """向 AIM Server 注册新 Agent"""
    async with ws_connect(server, open_timeout=10) as ws:
        payload = build_register_payload(agent_name, framework, operator_name, operator_id)
...
def main():
    ...
    parser.add_argument("--operator-id", default="OP0001", help="操作人 ID (默认 OP0001)")
    ...
    if args.register:
        ...
        result = asyncio.run(register_agent(
            args.server, args.agent_name, args.framework, args.operator, args.operator_id
        ))
        if result.get("cmd") == "register_ok":
            agent_id = result.get("global_id", result.get("agent_id", ""))
            agent_secret = result.get("agent_secret", "")
            if not agent_id:
                print(f"❌ 注册成功但未返回 agent_id，请检查服务端日志")
                return
            print(f"✅ 注册成功!")
