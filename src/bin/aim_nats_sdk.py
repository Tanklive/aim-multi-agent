...
class AIMNATSClient:
    ...
    async def register(
        ...
        """向 AIM Server 注册本 Agent

        使用 NATS request-reply 模式发送注册请求到 aim.reg.register。
        Server 返回注册确认（包含 agent_id 等）。
        超时或失败由调用方处理——不阻塞 Agent 启动。

        参考: proposals/register-optimization-v1.md
        """
        ...
        try:
            response = await self.nc.request(
                Subjects.reg_register(), data, timeout=timeout
            )
            result = parse_message(response.data)
            log.info(f"📝 [{self.agent_id}] 注册成功: {result.get('status', 'ok')}")
            return result
    ...
    async def emit_obs(self, status: str, msg_id: str = "", detail: str = "",
        ...
        if not js_ok or not use_jetstream:
            await self.nc.publish(subject, data)
